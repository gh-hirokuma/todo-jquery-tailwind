$(function () {
  ...
  const printCheckAll = () => {
    ...
    const checkAllButtonElem = `
      <button name="checkAllAction" class="w-10 h-10 bg-teal-300 rounded-sm shadow flex justify-center items-center text-2xl">
        ${checkIcon}
      </button>
    `
  ...
  const printTodoFilter = () => {
    ...
    const borderCompletedFilter = isCompletedFilter ? 'border-2 border-teal-300' : ''
  ...
  const toggleDoneTodo = (id) => {
    todoItems = todoItems.map((todo) => {
      if (todo.id == id) return { id: todo.id, name: todo.name, done: !todo.done }
      else return todo
    })
  }
